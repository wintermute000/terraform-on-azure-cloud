...
resource "azurerm_traffic_manager_endpoint" "tm_endpoint_project1_eastus2" {
  ...
  type                = "azureEndpoints"
  target_resource_id  = data.terraform_remote_state.project1-australiaeast-test.outputs.web_lb_public_ip_address_id
  weight              = 50
}
...
resource "azurerm_traffic_manager_endpoint" "tm_endpoint_project2_westus2" {
  ...
  profile_name        = azurerm_traffic_manager_profile.tm_profile.name
  type                = "azureEndpoints"
  target_resource_id  = data.terraform_remote_state.project2-australiasoutheast-test.outputs.web_lb_public_ip_address_id 
  weight              = 50
}
